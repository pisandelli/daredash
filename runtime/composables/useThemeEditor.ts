...
export function useThemeEditor(tabs: StudioTabDefinition[]) {
  ...
  function exportTokensJson(): string {
    ...
    // Determine if we are exporting the default theme or a specific one
    let target = overrides
    const currentTheme = activeThemeId.value

    if (currentTheme !== 'default') {
      overrides['themes'] = {
        [currentTheme]: {}
      }
      target = (overrides['themes'] as Record<string, any>)[currentTheme]
    }
