...
describe('DareDash Studio registry', () => {
  ...
  it('registers table with preserved token references', () => {
    ...
    const headerFontSizeField = tableTab?.fields.find((field) => field.path === 'table.header.font-size')
    const cellFontSizeField = tableTab?.fields.find((field) => field.path === 'table.cell.font-size')
    const largeDensityHeaderPaddingField = tableTab?.fields.find((field) => field.path === 'table.density.large.header.padding')
